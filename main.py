...
install_solc('0.8.0')  # Make sure version matches your pragma
...
def register():
    if request.method == 'POST':
        ...
        # Check which form field is being used for account type
        account_type = request.form.get('account_type')
        is_freelancer = (account_type == 'freelancer')

        # For debugging
        print(f"Registration data: account_type={account_type}, is_freelancer={is_freelancer}")

        skills = request.form.get('skills', '')
        wallet_address = request.form.get('wallet_address')

        # Create new user with the correct freelancer status
        user = User(
            username=username,
            email=email,
            is_freelancer=is_freelancer,  # Set based on form radio button
            skills=skills if is_freelancer else None,
...
def release_payment(task_id):
    ...
    # Get the freelancer user object
    freelancer = User.query.get(task.freelancer_id)

    # In a real application with MetaMask integration:
    # This would trigger the smart contract's releaseFunds function
    # which transfers ETH from the contract to the freelancer's wallet

    # For demo purposes, simulate blockchain transaction with hash
    tx_hash = '0x' + hashlib.sha256(f"{task.id}-{time.time()}".encode()).hexdigest()
    task.blockchain_tx_id = tx_hash

    # Important: Update task status to indicate payment has been released
    task.status = 'paid'  # You may need to add this status to your model
